...
MyGame = ig.Game.extend({
	...
	update: function() {
		...
		// ATOMIZER MODE
			...
			if (ig.input.pressed('test')){
			 	this.atomizer.updateInfo(); // mobile, x, y
			 }
	...
	draw: function() {
		...
		if (this.game_mode == 1 || this.game_mode == 2 || this.game_mode == 6) {
			...
			if (this.tutorial.tutNum == 1) { // arrow tips to indicate closing the atomizer
					this.drawTipArrow(505, 490);
			}
			if (this.tutorial.tutNum == 6) { // arrow tips to indicate using the breaker
					this.drawTipArrow(400, 490);
			}
			if (this.tutorial.tutNum == 4 || this.tutorial.tutNum == 3) { // arrow tips to indicate closing the atomizer
				if (this.atomizer.bondArray.length === 40 && !this.tutorial.closeTip){
					this.drawTipArrow(505, 490);
					// ig.game.unloadTip(ig.game.tip);
					// ig.game.loadTip(6, this.game.screen.x / 2, this.game.screen.y);
					// this.tutorial.closeTip = true;
				}
			}
...
//---->	MODIFY THIS TO TAKE X,Y INPUTS
