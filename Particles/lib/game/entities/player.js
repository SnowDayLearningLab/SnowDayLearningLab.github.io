...
EntityPlayer = ig.Entity.extend({
	...
	blockReact: function (other) {
		if (other){
			if (other.bounce >= 30 && this.lastVel.y > 0 && !this.bounced){
				if ( ig.input.state('jump') ) {
					this.bounced = true;
					this.jumpSound.play();
					this.vel.y = -this.jump + ((-this.lastVel.y * (other.bounce / 100)) - this.lastVel.y);
				} else {
					this.bounced = true;
					this.jumpSound.play();
					this.vel.y = (-this.lastVel.y * (other.bounce / 100)) - this.lastVel.y;
				}
			}
			if (other.slip > 0) {
				this.friction.x = 800 - (other.slip * 10); // 800 is default friction
			}
		} else {
			this.friction.x = 800;
		}
