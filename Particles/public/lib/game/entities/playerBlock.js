...
EntityPlayerBlock = ig.Entity.extend({
	...
	initProps: function(settings){
		if (this.thrower === "player") {
				...
				if (tips.length > 0){
					for (var i=0;i<tips.length;i++){
						if (this.touches(tips[i])){
							...
							} else if (tips[i].loc === "hard" && ig.game.tutorial.flag[8]){
								tips[i].kill();
								ig.game.tutorial.flag[8] = false;
								ig.game.tutorial.hidden = true;
							}
			...
			ig.game.logBlockEvent(this, "block_throw");
		} else {
	...
	removeTiles: function (fromDrop) {
		var tileNum = ig.game.backgroundMaps[2].getTile(this.pos.x, this.pos.y);
		if (tileNum != 0 && tileNum != 5){
			if(!fromDrop) {
				if (tileNum != 0){
					ig.game.logBlockEvent(this, "block_destroyed");
				}
			}
	...
	dropTile: function () {
		...
		if (tileNum != 0){
			...
			ig.game.logBlockEvent(this, "block_slipped");
		}
	...
	calcCrumb: function() {
		var perBonded = 1 - ((40 - this.totalBonds) / 40);
		if (perBonded<.5){
			this.crumbTime = new ig.Timer( perBonded + 2);
			console.log(this.crumbTime.delta());
		} else {
			console.log('hardness');
			if (this.hardness < 6 || this.totalBonds < 32){
				this.crumbTime = new ig.Timer( perBonded + this.hardness/2 + 2);
				console.log(this.crumbTime.delta());
			} else {
	...
	drawStructure: function (){
		...
		for (var i = 1, n = keys.length; i < n; ++i) {
			var k = keys[i];
			if (propObj[k] > propObj[max]) {
		       	max = k;
			}
		}
		if (this.totalBonds > 14) {
			if (max == "h" && this.hardness >6 && this.totalBonds > 31){
				this.drawHard();
			} else if (max == "b" && this.bounce >4){
				this.drawBounce();
			} else if (max == "s" && this.slip > 6){
				this.drawSlip();
			}
		}
	...
	update: function() {
		if (this.crumbTime){
			if (this.crumbTime.delta() > 0){
				...
				if (this.totalBonds > 14 && this.slip > 6){
					this.dropTile();
				} else {
					this.removeTiles();
				}
